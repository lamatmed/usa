...
export async function getVoteResults() {
  ...
  // Définir les résultats comme un objet avec des clés dynamiques de type string
  const resultsObj: { [key: string]: number; totalVotes: number } = {
    totalVotes,
  };

  // Remplir les résultats pour chaque choix
  results.forEach((result) => {
    resultsObj[result.choice] = result._count.choice; // Utilisation de result.choice comme clé dynamique
  });
