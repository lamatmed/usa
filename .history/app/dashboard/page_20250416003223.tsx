      ...
      <div className="container mx-auto px-4 py-12">
        ...
        >
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300">
            Bienvenue <span className="font-semibold text-blue-600 dark:text-green-400">{user.name}</span>
          </p>
        </motion.div>
  ...
  return (
    ...
    >
      <Card className={`h-full border-0 shadow-lg hover:shadow-xl transition-all hover:ring-2 hover:ring-${color}-400 dark:hover:ring-${color}-600 rounded-xl`}>
        <CardHeader>
          <CardTitle className={`flex items-center gap-3 text-b-600 dark:text-${color}-400`}>
            {icon}
            {title}
          </CardTitle>
