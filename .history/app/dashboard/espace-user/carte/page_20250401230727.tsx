    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-teal-400 to-blue-500 p-8">
      ...
      >
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-sm text-gray-600">Union des Sortants d&apos;Algérie</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col items-center text-center mt-4">
          {user.photo && (
            <Image src={user.photo} alt="Photo de profil" height={80} width={80} className="rounded-full border-4 border-blue-400 mb-4" />
          )}
          <p className="text-lg font-semibold text-blue-700">N° {user.nni}</p>
          <p className="text-xl font-bold text-blue-800 mt-2">{user.name}</p>
          <p className="text-md text-gray-700 mt-1">{user.job}</p>
          <p className="text-sm text-gray-500 mt-1">{user.domain}</p>
      ...
      <div className="mt-6 flex gap-4">
        <Button
          className="bg-teal-700 text-white px-6 py-3 rounded-lg hover:bg-teal-700 transition duration-300"
          onClick={() => handlePrint()}
        >
          Télécharger PDF
        </Button>
