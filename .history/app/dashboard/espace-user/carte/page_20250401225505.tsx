...
const MembershipCard = () => {
  ...
  const handlePrint = useReactToPrint({
    content: () => cardRef.current, // Référence pour l'impression
  });

  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-6">
      {/* Carte d'adhésion */}
      <div ref={cardRef} className="shadow-lg border rounded-lg p-4 bg-white w-80 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-12 bg-gradient-to-r from-blue-600 to-blue-400" />
        ...
        <CardContent className="flex flex-col items-center text-center mt-6 relative z-10">
          {user.photo && (
            <Image src={user.photo} alt="Photo de profil" height={70} width={70} className="rounded-full border-2 border-gray-300 mb-2" />
          )}
