...
const VotePage = () => {
  ...
  const [choices, setChoices] = useState<string[]>([]); // Tableau des choix récupérés

  // Récupérer les choix dynamiques au chargement de la page
  useEffect(() => {
    const fetchChoicesAndResults = async () => {
      const choicesData = await getChoices(); // Récupérer les choix
      setChoices(choicesData.map(choice => choice.label)); // Extraire seulement les labels
