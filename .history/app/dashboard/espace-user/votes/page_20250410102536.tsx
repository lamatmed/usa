...
const VotePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const choices = await getChoices();  // Récupérer les choix de vote
      const result = await getVoteResults(); // Récupérer les résultats des votes

      // Mettre à jour les résultats avec les votes et calculer les pourcentages
      const updatedChoices = choices.map((choice: any) => ({
        label: choice.label,
        votes: result[choice.label] || 0,  // Assurez-vous d'avoir les bons votes pour chaque choix
        percentage: 0,
      }));
