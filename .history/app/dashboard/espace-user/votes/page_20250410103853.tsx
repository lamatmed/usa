...
const VotePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Destructure the result into `totalVotes` and `choices`
      const { totalVotes, choices: resultChoices } = result;
  
      // Ensure totalVotes is not 0 before calculating percentages
      if (totalVotes === 0) {
        // If no votes have been cast, just return the choices with 0% and 0 votes
        const updatedChoices = resultChoices.map((choice) => ({
          label: choice.label,
          votes: 0,
          percentage: 0, // If no votes, the percentage will be 0
        }));
      ...
      // Initialize the updatedChoices array by mapping over resultChoices
      const updatedChoices = resultChoices.map((choice) => {
        // Find the corresponding vote count from the result
        const votes = resultChoices.find((item) => item.label === choice.label)?.votes || 0;
  
        return {
          label: choice.label,
          votes,
          percentage: (votes / totalVotes) * 100, // Calculate percentage
        };
      });
