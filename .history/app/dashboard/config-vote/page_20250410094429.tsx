...
const ConfigVotePage = () => {
  ...
  const handleAddChoice = async () => {
    ...
    if (result.success) {
      ...
      setChoices((prev) => [...prev, result.choice]); // Assurer que result.choice contient { id, label }
      setNewChoice("");
    } else {
  ...
  const handleDeleteChoice = async (choiceId: string) => {
    ...
    if (result.success) {
      ...
      setChoices((prev) => prev.filter((choice) => choice.id !== choiceId)); // Suppression basée sur l'id
    } else {
      toast({ title: "Erreur", description: result.message, variant: "destructive" });
    }
  };
