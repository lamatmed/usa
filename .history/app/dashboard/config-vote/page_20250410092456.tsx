...
import { getChoices, addChoice, deleteChoice } from "@/utils/vote-config";
...
const ConfigVotePage = () => {
  ...
  const handleDeleteChoice = async (choice: string) => {
    const result = await deleteChoice(choice);
    if (result.success) {
          ...
          <div className="space-y-2 mt-4">
            {choices.map((choice, index) => (
              <div key={index} className="flex justify-between items-center border p-2 rounded">
                <span>{choice}</span>
                <Button variant="destructive" onClick={() => handleDeleteChoice(choice)}>
                  Supprimer
                </Button>
              </div>
            ))}
