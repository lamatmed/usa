...
const navigation: Navigation = {
  ADMIN: [
    ...
    { name: "Membres", href: "/dashboard/users", icon: Users },
    { name: "Election", href: "/dashboard/config-vote", icon: Clipboard },
    { name: "Activités", href: "/activites/config", icon: NotebookPen },
  ],
  USER: [
    { name: "Tableau de bord", href: "/dashboard", icon: LayoutDashboard },
  ...
  return (
    <nav className="flex flex-col md:flex-row items-center md:space-x-6 space-y-2 md:space-y-0">
      {isAuthenticated && user ? (
        ...
        <span className="text-while (condition) {
          
        } text-sm">Connectez-vous pour accéder au menu</span>
      )}
    </nav>
  )
}
